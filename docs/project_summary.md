...
### 10.5 Raw trace 신뢰성 분석

`<<expr=result>>` 태그 내부 산술을 재계산 가능한 경우만 검사하면, 형식이 맞는 trace 중 상당수가 산술적으로도 일관되지 않았다.

| arm | 재계산 가능 태그 | 잘못된 태그 | 잘못된 태그 포함 row | 정답 row | 정답 row 중 태그 오류 포함 |
|-----|----------------:|------------:|---------------------:|---------:|---------------------------:|
| B1_ent | 398 | 185 | 123/500 | 13 | 2/13 |
| B1_skel | 215 | 90 | 31/500 | 11 | 1/11 |
| B2_ent | 771 | 298 | 204/500 | 14 | 4/14 |
| B2_ent_lr1e6 | 719 | 240 | 182/500 | 13 | 5/13 |
| B2_skel | 849 | 333 | 227/500 | 12 | 6/12 |
| B2_skel_lr1e6 | 727 | 212 | 153/500 | 14 | 1/14 |

대표 오류:
- `25 + 30 = 65`, `200 * 0 = 200`, `75 * 400 = 750`처럼 계산 태그 자체가 거짓이다.
- `6 + Razel = 12`처럼 자연어 엔티티를 숫자처럼 취급한다.
- `20%`를 비율로 해석하지 않고 정수 `20`으로 빼거나 곱한다.
- 일부 정답은 `####`가 없는데 마지막 숫자가 gold와 같아 fallback으로 맞은 것이다.

따라서 최종 acc도 “정상 reasoning 성공률”을 과대평가한다. `B2_skel`은 정식 `####` 정답 12개 전부가 hash 추출이지만, 그중 6개는 trace 내부 산술 오류를 포함했다. `B2_ent`도 정답 14개 중 4개는 fallback이고, hash 정답 중 일부도 의미/산술이 부정확하다.

정답 ID 겹침도 크다. 여섯 generation 파일 전체에서 한 번이라도 맞은 dev 문제는 42/500개뿐이고, 여러 arm에서 반복적으로 맞은 ID는 90, 49, 96, 240, 167, 244, 28 등 소수다. 즉 새 방법이 넓은 문제군을 풀기보다 쉬운/우연 문제를 서로 다르게 맞히는 경향이 강하다.
