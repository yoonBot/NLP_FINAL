#!/usr/bin/env python3
"""
DPO fine-tuning on top of a CoT-SFT checkpoint.

Requires: pip install trl transformers

Usage:
    python train_dpo.py --sft_checkpoint 9_10-1e-05-reasoning.pt --use_gpu

Input:
    data/dpo_pairs.jsonl   {prompt, chosen, rejected}  (from generate_rejected.py)

Output:
    outputs/dpo_gpt2_model/   HuggingFace-format model
"""
...
from reasoning_generation import seed_everything

DPO_PAIRS = os.path.join("data", "dpo_pairs.jsonl")
OUT_DIR = os.path.join("outputs", "dpo_gpt2_model")
